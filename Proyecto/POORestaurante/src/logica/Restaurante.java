...
public class Restaurante {
	...
	public void ingresarPedido(int idTipoPedido, int numerodeeleccion, String nombre,int precioVenta) {
	...
	public void imprimirFacturas() {
		for(Factura factura : this.facturas) {
			System.out.println("-------");
			System.out.println(factura.getNumero() + " -> " + factura.getFecha() + " -> "+ factura.getDescuento() + factura.getValorTotal()+ factura.getCajero().getNombre());
			for(FacturaPedido facturaPedido : factura.getFacturaPedidos()) {
				System.out.println(facturaPedido.getPedido().getNombre() + " -> " + facturaPedido.getCantidad() + " -> " + facturaPedido.getPrecio());
			}
		}
